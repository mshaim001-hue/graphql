#!/usr/bin/env python3
"""
GraphQL query script for group_user data
Executes group_user query and saves results to JSON file
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'errors' in data:
                ...
                return None
            return data.get('data', {}).get('group_user', [])
        else:
...
def save_results(results, filename=None):
    ...
    if not filename:
        ...
        filename = f"group_user_results_{timestamp}.json"
...
def main():
    """Main function"""
    print("Tomorrow School Group User Query Tool")
    print("=" * 40)
    ...
    print(f"Query successful! Found {len(results)} group_user records.")
    ...
    if filename:
        ...
        if results:
            ...
            for i, record in enumerate(results[:3]):
                group_info = record.get('group', {})
                user_info = record.get('user', {})
                print(f"  {i+1}. Group ID: {group_info.get('id')}, User: {user_info.get('login')}, Status: {group_info.get('status')}")
    else:
